# 📌 Vulnerability Reproduction (RCE via Malicious Plugin Upload)

## 🧾 Overview

This document describes the steps to reproduce a Remote Code Execution (RCE) vulnerability in the plugin upload functionality. The issue arises due to improper validation of ZIP archives in the `emUnZip()` function, allowing arbitrary PHP file upload and execution.
...
1. Log in to the application backend.
2. Navigate to the plugin upload/install feature.
3. Upload the crafted `eval.zip` file.
